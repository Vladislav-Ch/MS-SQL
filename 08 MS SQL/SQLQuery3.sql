...
as
begin
-- переменные для работы с данными, полученными из курсора
declare @Name_Region nvarchar(50), @Date date, @Temperature int, @Rainfall nvarchar(50);
	declare WeatherCur cursor
	for	
		select
	...
	fetch next from WeatherCur into @Name_Region, @Date, @Temperature, @Rainfall;
	...
	while @@FETCH_STATUS = 0 begin
		print 'Регион: ' +CONVERT(nvarchar(50), @Name_Region) + '; Дата: ' + CONVERT(nvarchar(10), @Date) + '; Температура: '+ CONVERT(nvarchar(15), @Temperature) + '; Осадки: ' + @Rainfall;
		fetch next from WeatherCur into @Name_Region, @Date, @Temperature, @Rainfall;
end
	close WeatherCur
	deallocate WeatherCur
end
go
